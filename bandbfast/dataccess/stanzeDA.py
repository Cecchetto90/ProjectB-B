...
def deleteStanza(stanza_id):
    try:
        """
        per eliminare la stanza ricerco con la query la stanza da eliminare, in seguito ottengo la key di quell'entitÃƒÂ , 
        key che ottengo con un metodo dal datastore. in seguito gli dico di eliminare l'entitÃƒÂ  identificata con quella key.
         con google datastore si elimina cosi, ÃƒÂ¨ una procedura standard
        """
...
def deleteAllStanza():
    try:
        """
        delete_multi ÃƒÂ¨ un metodo di app engine, ci permette di eliminare molte entitÃƒÂ ,
